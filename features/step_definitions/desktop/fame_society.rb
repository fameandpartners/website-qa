...
When(/^I open "The Fame Society Invitation" page\.$/) do
  on(HomePage) do |page|
    page.visit_site_version(country: 'USA', url: '', basic_auth: true)
    page.visit_site_version(country: 'USA', url: '/fame-society-invitation')
  end
end

Then(/^I open "The Fame Society Invitation" form\.$/) do
  on(FameSocietyPage).open_fame_society_invitation_form
end

And(/^sign up for "Fame Society Invitation"\.$/) do
  on(FameSocietyPage) do |page|
    full_name = Faker::Name.name
    first_name = Faker::Name.first_name
    last_name = Faker::Name.last_name
